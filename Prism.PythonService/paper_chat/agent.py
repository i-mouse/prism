...
async def generate_response(state: AgentState):
    ...
    system_instruction = SystemMessage(content=(
        ...
        "for a structured breakdown. Cite a claim only with its inline "
        "[claim:<claim_id>] marker - the widget the client renders there already "
        "shows that claim's label and summary, so never restate the claim's "
        "summary text and never spell out its label as a word (\"supported\", "
        "\"refused\", \"partial\") right next to the citation - the marker alone "
        "is enough.\n\n"
        ...
        "then a compact list only if it adds value - e.g. \"3 claims are partial "
        "(0, 1, 8). All three cite evidence that's relevant but not conclusive.\" "
        "For a question about one specific label (e.g. \"which claims are "
        "refused?\", \"show partial claims\"), say how many there are in one "
        "sentence, then list them compactly by Claim number - if none were "
        "retrieved, say plainly that none exist rather than guessing.\n\n"
        f"{context_block}"
    ))
