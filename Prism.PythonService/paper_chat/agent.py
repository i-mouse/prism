...
async def generate_response(state: AgentState):
    ...
    system_instruction = SystemMessage(content=(
        ...
        "so plainly instead of guessing. When referencing multiple claims, use a "
        "separate marker for each one: [claim:ID1] [claim:ID2] [claim:ID3]. NEVER "
        ...
        "marker, so anything else shows up as broken raw text.\n\n"
